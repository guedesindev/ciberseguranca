...
Hà a criação de duas chaves, uma chave pública para cifrar as mensagens, esta chave é pública todos que desejarem enviar uma mensagem pode ter acesso. É como um cadeado aberto que ao definir o conteúdo deve ser fecahdo. Outra chave é criada, diferente da primeira que é uma chave privada, apenas o receptor do conteúdo possui e pode abrir todos os cadeados destinados a ele. É como uma chave mestra. A existência dessas duas chaves torna o processo de criptografia e descriptografia mais seguro. A chave privada por sua vez sempre é criada por meio de processos matemáticos avançados utilizando conceitos de Logaritmos Discretos.

Logaritmo discreto é a operação inversa à exponenciação em um grupo finito, geralmente baseada em aritimética  modular. Matematicamente consiste em obter o expoente $x$ na expressão $g^x \equiv h (mod(p))$, onde g é a base, h é o resultado e p é um número primo.

Na matemática comum, encontrar $x$ na equação $g^x = h$ é feito usando logaritmos tradicionais. Na aritmética modular, entretanto, a operação "dá volta" repetidamente, criando uma função em zigue-zague. A operação de reversão da exponenciação em logaritmo discreto é conhecida por ser intratável na prática para números grandes, funcionando assim como uma via de mão única, por isso, este método matemático se torna apropriado para problemas de criptografia.

**Como Funciona?**

Imagine que uma pessoa quer enviar um pacote para outra, mas quer ter certeza que apenas o destinatário possa abrí-lo. Vamos supor que este pacote seja uma caixa. Aí o remetente diz para o destinatário: "Manda pra mim um cadeado que só você tenha a chave". O remetente recebe o cadeado e trava a caixa com este cadeado, cadeado este feito de um material que nenhuma ferramenta conhecida possa quebrar, nenhum pé-de-cabra possa forçá-lo e que fosse à prova da utilização de artefatos que forcem o segredo, ou que a quantidade de pinos do segredo fosse tão grande que ninguém conseguisse usar um grampo de aço para cabelo.
...
Para a criptografia assimétrica há dois algoritmos mais utilizados:

- [RSA - Rivest-Shamir-Adleman](rsa.md)
- [ECC - Ellipits Curves Cryptograpy](ecc.md)
